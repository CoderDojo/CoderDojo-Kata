...
function installToMediaWikiRoot {
	...
	if [ "$SWL" != "" ]
	then
		composer require 'mediawiki/semantic-watchlist='$SWL --prefer-source --update-with-dependencies
	else
		composer init --stability dev
		composer require mediawiki/semantic-watchlist "dev-master" --prefer-source --dev --update-with-dependencies
