	...
	*/
	...
	function getQueryTutorialsByCategory($namespace, $categories, $rating, $sklvl) {
		...
		*/
		...
		while ($obj = $dbr->fetchObject($res)) {
			array_push($languages, $obj->subcat);
			array_push($discardCategories, $obj->subcat);
		}
		...
		$q = array (
			...
			'fields' => array (
				...
				'txt.old_text as text',
				'txt.old_flags as text_flags',
				'count(distinct pl.pl_title) as sessions',
				...
				'pp.pp_value as templatedata'
			),
			// 'tables' => array ( 'cl' => 'categorylinks', 'kp' => 'page', 'pl' => 'pagelinks', 'allcat' => 'categorylinks' ),
			'tables' => array ( 'cl' => 'categorylinks', 'kp' => 'page', 'rev' => 'revision', 'txt' => 'text', 'pl' => 'pagelinks', 'skillLevel' => 'categorylinks', 'allcat' => 'categorylinks', 'rating' => 'w4grb_avg', 'pp' => 'page_props'),
			'join_conds' => array (
				'kp' => array (
				...
				'txt' => array (
					'LEFT JOIN', 'rev.rev_text_id = txt.old_id'
				),
				'pl' => array (
				...
				'pp' => array (
					'LEFT JOIN', 'kp.page_id = pp.pp_page and pp.pp_propname = \'templatedata\''
				)
			),
			'conds' => array (
	...
	function isTemplateDataItemEmpty($data, $itemName){
		return $data[$itemName] == wfMsg('katamentorstutoriallist_notavailable');
	}
	...
	function execute( $subPage) {
		...
		$res = $this->executeQuery($dbr, $this->getQueryTutorialsByCategory(NS_MENTOR_COURSE, array($topicSelection, $technologySelection), $ratingSelection, $levelSelection));
		$tutorials = array();
		while($obj = $dbr->fetchObject($res)) {
			...
			$tutorial = array(
				...
				'skillLevel' => $templatedata["level"], //'Beginner',
				'sessions' => $obj->sessions == 0 ? 1 : $obj->sessions,
				'category' => $obj->categories,
				'rating' => $obj->rating,
				'templatedata' => $templatedata,
				'content' => $contentPageData,
				'text' => Revision::decompressRevisionText($obj->text, split(" ", $obj->text_flags)),
			);
	...
	function printFilters($out, $filters) {
		...
		parse_str(parse_url($formUrl, PHP_URL_QUERY), $formParams);
		...
		foreach ($formParams as $key => $value) {
			$out->addHTML(
				Html::element('input', array('type' => 'hidden', 'value' => $value, 'name' => $key)). "\n"
			);
		}
		if (!empty($this->useskin)) {
			$out->addHTML(
	...
	function parseTemplateData($wikiPage){
		global $wgParser;
		$data = array();
		$data = SpecialKataTemplateParser::parseTemplate($wgParser, $this->context, $wikiPage, $this->parserTags, wfMsg('katamentorstutoriallist_notavailable'));
	...
	function appendTutorial($out, $tutorial) {
		...
		$courseUrl = $courseSpecialTitle->getLinkURL(array('page' => 'Mentor_Course:'.$tutorial['name']));
		...
		$out->addHTML(
			...
			Html::openElement('td', array( 'class' => 'kata-result-element kata-course-image', 'rowspan' => '6' )) . "\n".
			...
			Html::element('th', array( 'class' => 'kata-result-element kata-course-sessions' ), 'Sessions:') . "\n".
			Html::closeElement('tr') . "\n".
			
			Html::openElement('tr', array()) . "\n".
			Html::element('td', array( 'class' => 'kata-result-element kata-course-skill-level' ), $tutorial['skillLevel']) . "\n".
			Html::element('td', array( 'class' => 'kata-result-element kata-course-sessions' ), $tutorial['sessions']) . "\n".
			Html::closeElement('tr') . "\n".
			
			Html::openElement('tr', array()) . "\n".
			Html::element('th', array( 'class' => 'kata-result-element kata-course-category', 'colspan' => '2' ), 'Category') . "\n".
			Html::closeElement('tr') . "\n".
			
			Html::openElement('tr', array()) . "\n".
			Html::element('td', array( 'class' => 'kata-result-element kata-course-category', 'colspan' => '2' ), $tutorial['category']) . "\n".
			Html::closeElement('tr') . "\n".
	...
	function getImageUrl($imageName) {
		...
		if(filter_var($imageName, FILTER_VALIDATE_URL)) {
			$image = $imageName;
		} else if($imageName && $imageName != wfMsg('katamentorstutoriallist_notavailable')) {
